...
public class GlobalExceptionHandler {
    ...
    public ResponseInfo globleExceptionHandler(Exception e, HttpServletResponse response) {
        ...
        if (e instanceof MyException) {
            ...
            // 返回错误信息
            return new ResponseInfo().setMessage(e.getMessage()).setCode(ResultEnum.PARAMETER_ERROR.getCode());
        }
        // 判断是否为 NotFountResourceException 异常
        else if (e instanceof NotFountResourceException) {
            response.setStatus(HttpStatus.BAD_REQUEST.value());
            return new ResponseInfo().setCode(ResultEnum.NOT_FOUNT_RESOURCE.getCode()).setMessage(e.getMessage());
        }
        // 判断是否为丢失请求参数异常
        else if (e instanceof MissingServletRequestParameterException){
            response.setStatus(HttpStatus.BAD_REQUEST.value());
            return new ResponseInfo().setCode(ResultEnum.PARAMETER_MISSING_ERROR.getCode())
                    .setMessage(ResultEnum.PARAMETER_MISSING_ERROR.getMessage());
        }
        // 判断是否为缺少请求体异常
        else if (e instanceof HttpMessageNotReadableException){
            response.setStatus(HttpStatus.BAD_REQUEST.value());
            return new ResponseInfo().setCode(ResultEnum.REQUEST_MISSING_BODY_ERROR.getCode())
                    .setMessage(ResultEnum.REQUEST_MISSING_BODY_ERROR.getMessage());
        }
        // 判断是否为请求参数错误异常
        else if (e instanceof MethodArgumentNotValidException){
            response.setStatus(HttpStatus.BAD_REQUEST.value());
            return new ResponseInfo().setCode(ResultEnum.PARAMETER_ERROR.getCode())
